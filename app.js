    ...
    const firePoints = lines
      .map((line) => {
        ...
        if (
          ...
          if (confidence) {
            // Ignora a linha se a 'confiança' estiver vazia
            return {
...
function initThree() {
  ...
  ominousLight = new THREE.PointLight(0xff4500, 0, 15); // Cor laranja/vermelha, intensidade 0, alcance 15
  // Posiciona a luz na frente da América do Sul
...
function setupScrollAnimation() {
  ...
  ScrollTrigger.create({
    ...
    onEnter: async () => {
      ...
      if (cachedFireData.length > 0) {
        const analysis = analyzeFireDataByRegion(cachedFireData);
        const panel = document.getElementById("hotzone-panel");
        document.getElementById("hotzone-region").textContent =
          analysis.hotZone;
        gsap.to(panel, {
          duration: 1,
          right: 0,
      ...
      function addSparkBatch() {
        const batch = cachedFireData.slice(
          currentIndex,
  ...
  ScrollTrigger.create({
    ...
    onEnter: () => {
      ...
      const heatData = cachedFireData.map((point) => [
        point.lat,
        point.lon,
        0.5,
      ]);
      ...
      gsap
        .timeline()
        .to(
          sparksLayer.getLayers().map((l) => l.getElement()),
          { duration: 1, opacity: 0, ease: "power2.inOut" }
        )
        .from(
          heatLayer.getElement(),
    ...
    onLeaveBack: () => {
      ...
      gsap
        ...
        .to(
          sparksLayer.getLayers().map((l) => l.getElement()),
          { duration: 1.5, opacity: 0.7, ease: "power2.inOut" },
          "<"
        );
    },
  });
...
//painel zonas quentes

function analyzeFireDataByRegion(fireData) {
  const regionCounts = {
    Norte: 0,
    Nordeste: 0,
    "Centro-Oeste": 0,
  ...
  const regions = {
    Norte: { minLat: -17, maxLat: 6, minLon: -74, maxLon: -46 },
    Nordeste: { minLat: -18.3, maxLat: -1, minLon: -48.5, maxLon: -34 },
    "Centro-Oeste": { minLat: -24, maxLat: -7, minLon: -62, maxLon: -45 },
    Sudeste: { minLat: -25.5, maxLat: -14, minLon: -51.5, maxLon: -39 },
    Sul: { minLat: -34, maxLat: -22, minLon: -58, maxLon: -48 },
  };

  fireData.forEach((point) => {
    let found = false;
    ...
    if (!found) regionCounts.Outros++;
  });

  // Encontra a região com mais focos
  let maxFires = 0;
  let hotZone = "Nenhuma";
  for (const region in regionCounts) {
    if (regionCounts[region] > maxFires) {
      maxFires = regionCounts[region];
      hotZone = region;
    }
  }
  return { hotZone, count: maxFires };
}
