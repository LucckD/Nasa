...
async function getFireData(targetDateString) {
    ...
    try {
        ...
        console.log(`Encontrados ${firePoints.length} focos para ${targetDateString}.`);
...
function updateMapWithPoints(fireData) {
    if (!sparksLayer) { sparksLayer = L.layerGroup().addTo(map); }
    ...
    function addSparkBatch() {
        ...
        batch.forEach(point => { L.circleMarker([point.lat, point.lon], fireIcon).addTo(sparksLayer); });
    ...
    ScrollTrigger.create({
        ...
        onEnter: async () => {
            ...
            if (initialFireData.length > 0) {
              const calculatedIntensity = Math.min(initialFireData.length / 40000, 1.0);
              gsap.to(ominousLight, { intensity: calculatedIntensity, duration: 2.5, ease: "power2.out"});
            }
            ...
            gsap.to(counter, {
                duration: 2, value: initialFireData.length, ease: "power2.out",
                onUpdate: () => {
            ...
            toggleBtn.addEventListener('click', () => {
                ...
                toggleBtn.textContent = isHeatmapVisible ? "Mostrar Pontos Individuais" : "Analisar Densidade";
            });
            ...
            const formatDateLabel = (dateString) => new Date(dateString + 'T12:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' });
            ...
            slider.addEventListener('input', (event) => {
                ...
                currentDateLabel.textContent = (event.target.value == 6) ? "Hoje" : formatDateLabel(selectedDateString);
                
                clearTimeout(debounceTimer);
                debounceTimer = setTimeout(async () => {
                    const fireData = await getFireData(selectedDateString);
                    document.getElementById('fire-counter').textContent = fireData.length.toLocaleString('pt-BR');
                    if (isHeatmapVisible) {
                        updateMapWithHeatmap(fireData);
                    } else {
                        updateMapWithPoints(fireData);
        ...
        onLeaveBack: () => {
            ...
            gsap.timeline().to("#map-container", {
                opacity: 0, duration: 0.5,
                onComplete: () => { if (map) { map.remove(); map = null; } }
